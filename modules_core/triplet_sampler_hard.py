...
class TripletSampler(object):
    ...
    def sample_batch(self, output, y):
        ...
        for idx_anchor in range(0, y.size(0), self.args.triplet_positives): #one negative per positives
            ...
            positive_dist = None
            positive_y = None
            for idx_positive in range(idx_anchor + 1, idx_anchor + self.args.triplet_positives):
                each_y = y[idx_positive]
                if idx_positive != idx_anchor and torch.equal(each_y, anchor_y):
            ...
            negative_dist = None
            negative_y = None
            for idx_negative in range(0, y.size(0)):
                each_y = y[idx_negative]
                if idx_negative != idx_anchor and not torch.equal(each_y, anchor_y):
                    negatives_dist_all.append(anchor_distances[idx_positive])
                    if negative is None:
                        negative_y = each_y
